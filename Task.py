...
async def remote_stats(ws: WebSocket):
    ...
    try:
        while True:
            try:
                stats = {'type': 'remote-stats', 'cpu_temp': get_cpu_temp(), 'cpu_usage': get_cpu_usage(), 'storage_total': get_storage_usage()['total'], 'storage_used': get_storage_usage()['used'], 'camera_status': State.camera_status, 'microphone_status': State.microphone_status, 'video_frame_age_ms': State.video_frame_age_ms, 'latest_capture_ts_ms': State.latest_capture_ts_ms}
                try:
